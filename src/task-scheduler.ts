...
export type TaskTimeoutHandler = (
	taskId: TaskId,
	taskLogger: Logger,
) => Promise<void>;
...
export function makeTaskScheduler(
	logger: Logger,
	params: { intervalMs: number; maxConcurrentTasks: number },
): TaskScheduler {
	...
	const registerTask = (
		task: Task,
	): { taskId: TaskId; scheduledTime: Date } => {
	...
	const executeTaskAsync = async (taskId: TaskId) => {
		...
		try {
			...
			const timeoutPromise = new Promise<TaskResult>(resolve => {
				setTimeout(() => {
					task.onTimeout?.(taskId, taskLogger).catch((err) => {
						taskLogger.error(
							`Failed to execute onTimeout handler: ${err.message}`,
							{ error: err },
						);
					});
			...
			logger.error(`Failed to execute task ${taskId}: ${message}`, {
				error,
			});
		} finally {
	...
	const startScheduler = async () => {
		...
		for await (const _ of setInterval(intervalMs, {
			...
			const dueTasks = Array.from(scheduledTasks)
				.filter(
					([_taskId, task]) =>
						task.scheduledTime.getTime() - now.getTime() <= intervalMs,
				)
				.sort(
					(a, b) => a[1].scheduledTime.getTime() - b[1].scheduledTime.getTime(),
				)
				.slice(0, availableSlots)
				.map(([taskId, _task]) => taskId);

			logger.debug(
				`Executing ${dueTasks.length} / ${scheduledTasks.size} tasks at ${now.toUTCString()}`,
				{ dueTasks, now },
			);
	...
	startScheduler().catch((error: unknown) => {
		const message = error instanceof Error ? error.message : String(error);
		logger.error(`Scheduler failed: ${message}`, { error });
		process.exit(1);
	});
